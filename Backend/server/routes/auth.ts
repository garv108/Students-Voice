...
export function registerAuthRoutes(app: Express) {
  ...
  app.post("/api/auth/signup", async (req, res) => {
    try {
      ...
      const user = await storage.createUser({
        ...
        // isVerified will default to false in DB, but we won't check it
      });

      // RESEND: disabled – no token generation, no email sending
      // const verificationToken = crypto.randomBytes(32).toString('hex');
      // const tokenExpiry = new Date(Date.now() + 24 * 60 * 60 * 1000);
      // await storage.setVerificationToken(user.id, verificationToken, tokenExpiry);
      // const emailSent = await sendVerificationEmail({...});

      // Auto-login (bypass email verification)
  ...
  // RESEND: VERIFY EMAIL – completely disabled (return 404)
  app.get("/api/auth/verify-email", async (_req, res) => {
  ...
  app.post("/api/auth/login", async (req, res) => {
    try {
      ...
      // RESEND: disabled – no email verification check
      // const isVerified = await storage.isUserVerified(user.id);
      // if (!isVerified) { ... }

      if (user.bannedUntil && new Date(user.bannedUntil) > new Date()) {
        return res.status(403).json({
          message: `Account banned until ${new Date(user.bannedUntil).toLocaleDateString()}`
        });
      }
